...
int main(int argc, char **argv)
{
    ...
    memset(anew, 0, (by + 2) * (by + 2) * sizeof(double));
    ...
    for (iter = 0; iter < niters; ++iter) {
        ...
        MPI_Isend(&aold[ind(1, 1)],  bx, MPI_DOUBLE,     north, 9, MPI_COMM_WORLD, &reqs[0]);
        MPI_Isend(&aold[ind(1, by)], bx, MPI_DOUBLE,     south, 9, MPI_COMM_WORLD, &reqs[1]);
        ...
        MPI_Irecv(&aold[ind(1, 0)],    bx, MPI_DOUBLE,     north, 9, MPI_COMM_WORLD, &reqs[4]);
        MPI_Irecv(&aold[ind(1, by+1)], bx, MPI_DOUBLE,     south, 9, MPI_COMM_WORLD, &reqs[5]);
